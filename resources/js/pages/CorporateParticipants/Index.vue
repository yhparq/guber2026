...
import { Edit, Trash2, Eye, X, Users, FileDown } from 'lucide-vue-next';
import { ref } from 'vue';
...
const deleteParticipant = (id: number) => {
    if (confirm('¿Estás seguro de que deseas eliminar este participante corporativo?')) {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold">Lista de Participantes Corporativos</h1>
                <div class="flex gap-2">
                    <a href="/corporate-participants/export" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-bold rounded-md transition-colors shadow-sm">
                        <FileDown class="w-4 h-4 mr-2" /> Exportar Excel
                    </a>
                    <Button as-child>
                        <Link href="/corporate-participants/create">Nuevo Registro</Link>
                    </Button>
                </div>
            </div>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead class="bg-gray-100 dark:bg-gray-800 text-xs uppercase font-bold text-gray-600 dark:text-gray-300 border-b border-sidebar-border/70 dark:border-sidebar-border">
                            <tr>
                                ...
                                <th class="px-4 py-3 whitespace-nowrap">Empresa / Pago</th>
                                ...
                                <th class="px-4 py-3 whitespace-nowrap">Facturación Ind.</th>
                        ...
                        <tbody class="divide-y divide-sidebar-border/70 dark:divide-sidebar-border">
                            <tr v-for="p in participants.data" :key="p.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <div class="font-bold text-gray-900 dark:text-white">{{ p.razon_social }}</div>
                                    <div class="text-xs text-gray-500 font-mono">Op: {{ p.codigo_pago }}</div>
                                    <button v-if="p.foto_voucher" @click="openVoucher(p.foto_voucher)" class="text-xs text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1 mt-1">
                                        <Eye class="w-3 h-3" /> Ver Voucher
                                    </button>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="font-bold text-gray-900 dark:text-white">{{ p.nombres }} {{ p.apellidos }}</div>
                                    <div class="text-xs text-gray-500 font-mono">{{ p.dni }}</div>
                                ...
                                <td class="px-4 py-3">
                                    <div class="truncate max-w-[150px]" :title="p.direccion">{{ p.direccion }}</div>
                                ...
                                <td class="px-4 py-3">
                                    <div v-if="p.tipo_comprobante_individual === 'Factura'">
                                        <span class="text-xs font-bold text-gray-700">Factura</span>
                                        <div class="text-xs text-gray-500">RUC: {{ p.ruc_individual }}</div>
                                    </div>
                                    <span v-else-if="p.tipo_comprobante_individual" class="text-xs text-gray-500">{{ p.tipo_comprobante_individual }}</span>
                                    <span v-else class="text-xs text-gray-400 italic">Global</span>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <Link :href="`/corporate-participants/${p.id}/edit`" class="p-2 hover:bg-gray-100 rounded-lg text-gray-500 hover:text-blue-600 transition-colors" title="Editar">
                                            <Edit class="w-4 h-4" />
                                        </Link>
                            ...
                            <tr v-if="!participants.data || participants.data.length === 0">
                                <td colspan="9" class="px-4 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        ...
                                        <p>No se encontraron participantes corporativos registrados.</p>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
